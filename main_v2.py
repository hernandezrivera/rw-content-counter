...
def call_inoreader(items, n_items, date_unix, continuation, non_read, match_rw, time_start):
    ...
    yield render_template('wait.html', date=date_unix, non_read=non_read, n_inoreader=len(items),
                          n_reliefweb=rw_n_items, time=total_minutes, categories=categories)
    if 'continuation' not in contents:
        return None
    else:
        return contents['continuation']
...
def post_something():
    ...
    items = []
    categories = {}
    call = call_inoreader(items, n_items, date_unix, continuation, non_read, match_rw, time_start)
    while True:
        continuation = call.next()
        a = dumb()
        print(
            '%i items read from inoreader.' % len(items))
        if (continuation is None) | debug | ((len(items) > max_items) & (max_items != 0)):
            break

    print('There are %i contents from inoreader ' % (len(items)))
    print('There are %i contents with matching with  ReliefWeb ' % rw_n_items)
    ...
    try:
        df.to_csv(r'data' + time_end + '.csv', index=None)
    except:
        print("I cannot write in the output file, omitting")

    print(categories)
    return render_template('results.html', date=date, non_read=non_read, n_inoreader=len(items),
                           n_reliefweb=rw_n_items, time=total_minutes, categories=categories)
